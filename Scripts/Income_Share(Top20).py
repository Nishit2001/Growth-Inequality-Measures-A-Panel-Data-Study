"""
Income Share (Top 20%, WID) — Data Transformation Pipeline

Extracted from Income_Share(Top20).ipynb: only the steps that modify/produce
the final dataframe are kept here. Exploratory cells (df.info, nunique,
value_counts) and all outlier-detection/plotting cells used for visual
data-quality review have been excluded — see Income_Share(Top20).ipynb if
...
# ---------------------------------------------------------------------------
# 1. Load and merge raw WID data (4 regional extracts covering countries A-B,
#    C-H, I-M, N-Z)
# ---------------------------------------------------------------------------
file_paths = [
    "../Data/Raw/Income_Share(Top20)/Income_share(A_B)/WID_Data_14072026-081824.csv",
    "../Data/Raw/Income_Share(Top20)/Income_share(C_H)/WID_Data_14072026-082425.csv",
    "../Data/Raw/Income_Share(Top20)/Income_share(I_M)/WID_Data_14072026-083100.csv",
    "../Data/Raw/Income_Share(Top20)/Income_share(N_Z)/WID_Data_14072026-084649.csv",
]
...
# Removing null values since Value column has lots of nulls for the earliest
# years of data
...
# ---------------------------------------------------------------------------
# Note on outlier handling (see Income_Share(Top20).ipynb for the full
# review): a bounds check (Value outside [0.20, 1.00]) and per-country IQR
# outlier detection were run on Value, then cross-checked against the
# gini_index outlier list and visually reviewed via time-series plots.
# No data points were dropped as a result of that review — it is
# exploratory/plotting-based, not a transformation, so it is intentionally
# omitted here.
# ---------------------------------------------------------------------------

# ---------------------------------------------------------------------------
# 5. Data Quality Note — Early Years (1940-1960) excluded
...
# ---------------------------------------------------------------------------
df_filtered_final.to_csv("../Data/Processed/CSV_data/Income_Share(Top20).csv", index=False)
df_filtered_final.to_parquet("../Data/Processed/Parquet_data/Income_Share(Top20).parquet", index=False)
